...
[ForceDisable("弃用, 暂未动工, 打算使用Steamworks作为后端, 不再使用自建服务器")]
...
internal class LiveLobbyList : Feature
{
    ...
    public class LiveLobbyQueryEntry : LiveLobbyQueryBase
    {
        public LiveLobbyQueryEntry()
        {
            if (PromptLabel.HasPrimaryText)
                PromptLabel.PrimaryText.Cast<TextMeshPro>().SetText(Localization?.GetById(4, "搜索") ?? string.Empty);
            PromptLabel.LabelText = Localization?.GetById(4, "点按下方按钮搜索符合条件的大厅") ?? string.Empty;

            QueryButton = new(Localization?.GetById(1) ?? "搜索", null, () =>
            {
                Task.Run(async () =>
                {
                    PromptLabel.PrimaryText.Cast<TextMeshPro>().SetText(Localization.GetById(2, "正在搜索..."));
                    ...
                    PromptLabel.PrimaryText.Cast<TextMeshPro>().SetText(Localization.Format(3, "已搜索到{0}个符合条件的大厅", Settings.LiveLobbyEntries.Count));
                });
            });
        }
    ...
    public class LiveLobbyEntry
    {
        public LiveLobbyEntry(LiveLobby lobby)
        {
            ...
            JoinButton = new(Localization.GetById(5, "加入"), null, () =>
            {
